    ...
    def image_callback(self, msg):
        if self.recording:
            ...
            if self.frame_count % 2 == 0:  # Adjust the modulus value based on desired rate
                ...
                if self.gripper_width == 0.04 :
                    self.gripper_width = 1
                elif self.gripper_width == 0.028:
...
class PandaRobotTask:
    ...
    def perform_task(self):
        while self.success_count < TOTAL_EPISODES:
            ...
            # self.set_box_position_from_input()


            self.new_x = self.new_x + random.uniform(-0.1, 0.1)
            self.new_y = self.new_y + random.uniform(-0.05, 0.1)
